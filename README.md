...
**混合检索 (RAG)**：搭建 BM25 + 稠密向量 (Milvus) 双路召回架构，微调 Qwen3-0.6B 作为专用 Embedding 模型。在 DISC-law 数据集上，Recall@10 从纯向量的 79% 提升至 85%。
...
2、
3、修改.env_temp文件为.env文件，主要写一个环境配置，具体参数在文件里面有相应描述。
4、
